...
@section('content')
    <div class="row">
        <div class="col-12">
            <form method="post" action="{{route('featured-category.update',$featuredCategory->id)}}" enctype="multipart/form-data">
                @csrf
                @method('patch')
                <div class="card">
                    <div class="card-header card_header">
                        <div class="row">
                            <div class="col-md-8">
                                <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> Update Featured Category
                                </h4>
                            </div>
                            <div class="col-md-4 text-right">
                                <a href="{{route('featured-category.index')}}"
                                   class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i
                                        class="fas fa-th label-icon"></i>All Featured Category</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="col-sm-7">
                                <select class ="form-control select2" id ="parentId" name="category_id">
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id}}" style="font-size: 12px; font-weight:bold !important;" {{ $category->id == $featuredCategory->category_id ? 'selected' : '' }} > {{  $category->category_name}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('effectiveDate') ? ' has-error' : '' }}">
                            ...
                            <div class="col-sm-7">
                                <input type="date" autocomplete="off" class="form-control form_control" name="effectiveDate"
                                       value="{{old('effectiveDate',$featuredCategory->effectiveDate)}}">

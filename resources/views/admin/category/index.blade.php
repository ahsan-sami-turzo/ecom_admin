        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header card_header">
                    <div class="row">
                        <div class="col-md-8">
                            <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Category Information
                            </h4>
                        </div>
                        <div class="col-md-4 text-right">
                            <a href="{{ route('category.create') }}"
                               class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i
                                    class="fas fa-plus-circle label-icon"></i>Add Category</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <table id="alltableinfo"
                        ...
                        @foreach($categories as $category)
                            <tr>
                                ...
                                <td>
                                    @foreach($category->parentCategory as $sub)
                                        {{ $sub->category_name."\n  ," ?: '' }}
                                    @endforeach
                                </td>
                                <td>
                                    @if($category->is_color)
                                        <label
                                            class="btn btn-sm btn-pink"> {{ $category->is_color == 1 ? "Color": '' }}</label>
                                    @endif
                                    @if($category->is_size)
                                        <label
                                            class="btn btn-sm btn-info"> {{ $category->is_size == 1 ? "Size": '' }}</label>
                                    @endif
                                    @if($category->is_weight)
                                        <label
                                            class="btn btn-sm btn-warning"> {{ $category->is_weight == 1 ? "Weight": '' }}</label>
                                    @endif
                                </td>
                                <td><label
                                    class="btn btn-sm {{ $category->status == 'active' ? 'btn-success' : 'btn-danger' }}"> {{ $category->status == 'active' ? "active": 'Inactive' }}</label></td>

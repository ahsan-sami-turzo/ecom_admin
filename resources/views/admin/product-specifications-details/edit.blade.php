...
@section('content')
    <div class="row">
        <div class="col-12">
            <form method="post" action="{{ route('product-specification-details.update',$product_specification_detail->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="card">
                    <div class="card-header card_header">
                        <div class="row">
                            <div class="col-md-8">
                                <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> Update product specification details</h4>
                            </div>
                            <div class="col-md-4 text-right">
                                <a href="{{ route('product-specification-details.index') }}" class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i class="fas fa-th label-icon"></i>All product specification details</a>
                            </div>
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-sm-5">
                                <select class="form-control select2" id="categoryId" name="category_id">
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id}}" {{ $category->id == $product_specification_detail->category_id ? 'selected' : '' }}
                        ...
                        <div class="form-group row mb-3">
                            <label class="col-sm-4 col-form-label col_form_label text-left">Specification Details Name<span
                                    class="req_star">*</span>:</label>
                            <div class="col-sm-5">
                                <select class="form-control tags" multiple="multiple" name="specification_details_name[]">
                                    @isset($product_specification_detail->specification_details_name)
                                        @foreach(json_decode($product_specification_detail->specification_details_name)  as $key => $val )

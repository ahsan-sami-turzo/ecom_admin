...
@section('content')
    <form action='{{ route("purchase-return.store") }}' method="post" class="dropzone" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <div id="product-entry-form" class="card-body d-none">
                        <table id="alltableinfo"
                            ...
                            <tr class="addRowTr">
                                ...
                                <td class="text-center">
                                    <div class="btn-group">
                                        <button class="btn btn-dark btn-sm removeRowBtn" id="addRowBtn" type="button"><i
    ...
    <script>
        $(document).ready(function () {
                                ...
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        ...
                                        if (jQuery.inArray(purchaseDetails.purchase_details)) {
                                            var add_row = $(".addRowTr").first().clone();
                                            ...
                                            $.each(purchaseDetails.purchase_details, function (index, value) {
                                                //$('#alltableinfo tr:first').find('#productId value').val('asda');
                                                $('#alltableinfo tr:first').parent().parent().find(".purchaseId").eq(index).val(value.purchaseId);
                                                $('#alltableinfo tr:first').parent().parent().find(".product-from-id").eq(index).val(value.product_entry_id);
                                                $('#alltableinfo tr:first').parent().parent().find(".productId").eq(index).val(value.productId);
                                                $('#alltableinfo tr:first').parent().parent().find(".product-from").eq(index).val(value.product.product_name);
                                                $('#alltableinfo tr:first').parent().parent().find(".colorId").eq(index).val(value.colorId);
                                                $('#alltableinfo tr:first').parent().parent().find(".color-name").eq(index).val(value.color.name);
                                                $('#alltableinfo tr:first').parent().parent().find(".sizeId").eq(index).val(value.sizeId);
                                                $('#alltableinfo tr:first').parent().parent().find(".size-name-id").eq(index).val(value.size.name);
                                                $('#alltableinfo tr:first').parent().parent().find(".size-name").eq(index).val(value.size.name);
                                                $('#alltableinfo tr:first').parent().parent().find(".weightId").eq(index).val(value.weight_id);
                                                $('#alltableinfo tr:first').parent().parent().find(".weight-name").eq(index).val(value.weight.name);
                                                $('#alltableinfo tr:first').parent().parent().find(".productQuantity").eq(index).val(value.quantity);
                                                $('#alltableinfo tr:first').parent().parent().find(".productPrice").eq(index).val(value.price);
                                                $('#alltableinfo tr:first').parent().parent().find(".totalPrice").eq(index).val(value.quantity * value.price);
                                                $('#alltableinfo tr:first').parent().parent().find(".removeRowBtn").eq(index).html(htmlBtn);
            ...
            $(".removeRowBtn").on('click', function () {
                let dfd = $(this).parent().parent().parent().remove();
                console.log(dfd);

                    ...
                    <div id="product-entry-form" class="card-body d-none">
                        <table id="alltableinfo"
                            ...
                            <tr class="addRowTr">
                                ...
                                <td class="text-center" style="width: 5%"><input  onkeyup="test()" style="height: 28px; padding: 0;"
                                                                                 type="text"
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#vendorId').on('change', function () {
                ...
                // Purchase Return Bill No

                $.ajax({
                    method: "post",
                    url: '{{ route("vendor-purchase-return-list") }}',
                    data: {
                        '_token': '{{ csrf_token() }}',
                        vendor_id: vendorId
                    },
                    success: function (response) {
                        ...
                        let count = vendor.purchases_return.length;
                        let pad = '00000';
                        count++;
                        let ctxt = '' + count;
                        //let petten = (vendor.name. slice(0, 3)+'-'+'000'.substr( String(vendor.id).length ) + vendor.id + '-' + pad.substr(0, pad.length - ctxt.length));
                        let petten = 'RETURN-' + vendor.name.toUpperCase().slice(0, 3) + '-' + '000'.substr(String(vendor.id).length) + vendor.id + '-' + (pad.substr(0, pad.length - ctxt.length) + count);

                        $('#purchaseReturnBillNo').val(petten);
                    },
                });
            });

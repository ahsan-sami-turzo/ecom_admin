...
@push('scripts')
    ...
    <script>
        $(document).ready(function () {
            $('#vendorId').on('change', function () {
                ...
                $('#billNumbers').on('change', function () {
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            const pDetails = (response.success);
                            if (jQuery.inArray(pDetails.purchase_details)) {
                                $("#totalQuantity").val(pDetails.totalQuantity);
                                $("#totalAmount").val(pDetails.totalAmount);
                                $.each(pDetails.purchase_details, function ( index,value) {

                                    $('#alltableinfo tr:last').after(`
                                     <tr class="addRowTr">
                                <td class="text-center" style="width: 25%">
                                    <input hidden class="form-control product-from-id"  name="product_entry_id[]"
                                           value="`+value.product_entry_id+`"/>
                                    <input class="form-control productId" value="`+value.product.product_name+`"/>
                                    <input hidden class="form-control product-from" id="productId" value="`+value.productId+`"/>

                                </td>
                                <td class="text-center" style="width: 15%">
                                    <input class="form-control colorId" hidden id="" value="`+value.colorId+`"/>
                                    <input class="form-control color-name" id="colorId" value="`+value.color.name+`"/>

                                </td>
                                <td class="text-center" style="width: 13%">
                                    <input hidden class="form-control sizeId" name="size_id[]" value="`+value.sizeId+`"/>
                                    <input class="form-control size-name" id="sizeId" value="`+value.size.name+`"/>
                                </td>
                                <td class="text-center" style="width: 12%">
                                    <input class="form-control weightId" hidden name="weight_id[]" value="`+value.weight_id+`"/>
                                    <input class="form-control weight-name " id="weightId" value="`+value.weight.name+`"/>
                                </td>
            ...
            $(".productQuantity").on('click', function () {
                getQunty();
            });

        });

        function getQunty(productEntryId = null,productId = null, colorId = null, sizeId = null, weightId = null, productQuantity) {

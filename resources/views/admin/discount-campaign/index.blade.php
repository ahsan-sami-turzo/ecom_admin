                ...
                <div class="card-header card_header">
                    <div class="row">
                        ...
                        <div class="col-md-4 text-right">
                            <a href="{{ route('discount-campaign.create') }}"
                               class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i
                                    class="fas fa-plus-circle label-icon"></i>Add discount campaign</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <table id="alltableinfo"
                        ...
                        <tbody>
                        @foreach($discountCampaigns as $discount)
                            <tr>
                                <td>{{ $discount->name ?: '' }}</td>
                                <td> <label class=" {{ $discount->campagin_discount_type == 'taka' ? 'text-success' : 'text-pink' }} ">{{ $discount->campagin_discount_type ?: '' }}</label> </td>
                                <td>{{ $discount->percentage ?: '' }}</td>
                                <td>{{ $discount->amount ?: '' }}</td>
                                <td>{{ $discount->effective_from ?: '' }}</td>
                                <td>{{ $discount->effective_to ?: '' }}</td>
                                <td> <button type="button" class="btn btn-sm  {{ $discount->status == 1 ? "btn-success": 'btn-pink' }}">{{ $discount->status == 1 ? "active": 'Inactive' }}</button> </td>
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <a class="dropdown-item" href='{{route("discount-campaign.edit",$discount->id)}}'>Edit</a>

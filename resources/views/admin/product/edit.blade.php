...
@section('content')
    <form action='{{ route("product.update",$product->id) }}' method="post" class="dropzone"
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                            ...
                            <div style="padding-left: 0 !important;" class="col-sm-6">
                                @if(count($product->color) > 0)
                                    @foreach($product->color as $key => $value)
                                        <div style="margin-left: -20px" class="row mb-3 colorRow" id="colorRow">
                                            <div style="padding-left: 32px !important;" class="col-sm-5">
                                                <div class="input-group mb-3">
                                                    <input id="colorId" name="color_id[]" value="{{ $value->id }}"
                                                           hidden>
                                ...
                                @if(count($product->weight) > 0)
                                    @foreach($product->weight as $key => $value )
                                        <div class="input-group mb-3 inputFormRow"
                                            ...
                                            <input style="margin-right: 10px; width: 100px !important;" type="text"
                                                   autocomplete="off"
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('isApprove') ? ' has-error' : '' }}">
                            <div class="col-sm-6">
                                <input type="checkbox"
                                       name="isApprove" hidden value="{{ $product->isApprove  }}">

                            </div>
                        </div>
                        <input id="colorObjects" hidden name="removeColorObjects">
                        <input id="sizeObjects" hidden name="removeSizeObjects">
                        <input id="weightObjects" hidden name="removeWeightObjects">
                    </div>
        ...
        $(document).ready(function () {
            ...
            $('#addSize').on('click', function () {
                let add_size_div = `<div class="input-group mb-3 inputFormRow" style="width: 100% !important;">
                                    <input id="sizeId" name="size_id[]" value="" hidden>
                ...
                $('.inputFormRow:last-child').after(add_size_div);
...
<button class="btn btn-outline-danger removeWeight" type="button"><i class="fa fa-minus"></i></button>
            ...
            var i = {{ count($product->color) }};
            $('#addColor').on('click', function () {
                // let add_color_div = ``;

                $(this).parents('#colorRow').clone();
                $('.colorRow:last-child').after(`<div style="margin-left: -20px" class="row mb-3 colorRow">
                    <div style="padding-left: 32px !important;" class="col-sm-5">
                        <div class="input-group mb-3">
                            <input name="color_id[]" value="" hidden>
                            <input  type="text" autocomplete="off" class="form-control form_control"
        ...
        $(document).ready(function () {
            let sizeObjects = [];
            $(".removesize").on('click', function () {
                let sizeId = $(this).parent().parent().find('#sizeId').val();

                let productSizeObject = {
                    'product_id': {{ $product->id }},
                    'size_id': sizeId,
                    'softDel': 0,
                }
                sizeObjects.push(productSizeObject);
                $(this).parent().remove();
                $('#sizeObjects').val(JSON.stringify(sizeObjects));
            });

            $(".removeWeight").on('click', function () {
                let weightId = $(this).parent().find('#weightId').val();

                let productWeightObject = {
                    'product_id': {{ $product->id }},
                    'weight_id[]': weightId,
                    'softDel': 0,
                }
                weightObjects.push(productWeightObject);
                $(this).parent().parent().remove();
                $('#weightObjects').val(JSON.stringify(weightObjects));

            });

            let colorObjects = [];
            $(".removeColor").on('click', function () {
                let colorId = $(this).parent().parent().find('#colorId').val();

                let productColorObject = {
                    'product_id': {{ $product->id }},
                    'color_id': colorId,
                    'softDel': 0,
                }
                colorObjects.push(productColorObject);

                $(this).parent().parent().remove();
                $('#colorObjects').val(JSON.stringify(colorObjects));
            });
        });
    </script>
@endpush

...
@section('content')
    <div class="row">
        <div class="col-12">
            <form method="post" action="{{ route('size-info.update',$sizeInfo->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="card">
                    <div class="card-header card_header">
                        <div class="row">
                            ...
                            <div class="col-md-4 text-right">
                                <a href="" class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i class="fas fa-th label-icon"></i>All size Info</a>
                            </div>
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-sm-5">
                                <input type="text" class="form-control form_control @error('name') is-invalid @enderror"
                                       name="size" value="{{old('size',$sizeInfo->size)}}">
                                {{--@if ($errors->has('title'))
                                ...
                                <select class="form-control select2" id="parentId" name="size_type_id">
                                    @foreach ($sizeTypes as $sizeType)
                                        <option value="{{ $sizeType->id}}"
                                                style="font-size: 12px; font-weight:bold !important;" {{ $sizeType->id == $sizeInfo->size_type_id ? 'selected' : '' }} > {{  $sizeType->name ?: ''}}</option>
                                    @endforeach
                                </select>

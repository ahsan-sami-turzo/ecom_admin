...
@section('content')
    <div class="row">
        <div class="col-12">
            <form method="post" action="{{ route('feature-product.update',$feature_product->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('display_products') ? ' has-error' : '' }}">
                            ...
                            <div class="col-sm-5">
                                <select class="form-control display-product" id="displayProducts" name="display_products">
                                    <option value=""></option>
                                    <option value="7" {{ $feature_product->display_products == 7 ? 'selected' : '' }}>Feature Products (7)</option>
                                    <option value="5" {{ $feature_product->display_products == 5 ? 'selected' : '' }}>Slider Products (5)</option>
                                    <option value="3" {{ $feature_product->display_products == 3 ? 'selected' : '' }}>Selected Products (3)</option>
                                    <option value="2" {{ $feature_product->display_products == 2 ? 'selected' : '' }}>Special Products (2)</option>
                                </select>
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('feature_id') ? ' has-error' : '' }}">
                            <label class="col-sm-4 col-form-label col_form_label text-left">Feature Name {{ $feature_product->feature_id }}<span
                                    class="req_star">*</span>:</label>
                            <div class="col-sm-5">
                                <select class="form-control " id="featureName" name="feature_id">
                                    <option value=""></option>
                                    @foreach( $featureNames as $featureName)
                                        <option value="{{ $featureName->id }}"  {{ $feature_product->feature_id == $featureName->id ? 'selected' : ''   }}>{{ $featureName->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-5">
                                <select class="form-control productName" id="parentId" name="product_id[]" multiple>
                                    ...
                                    @foreach ($products as $product)
                                        ...
                                        @foreach(json_decode($feature_product->product_id) as  $vl)
                                            {{ $product->id == $vl ? 'selected' : ''  }} @endforeach

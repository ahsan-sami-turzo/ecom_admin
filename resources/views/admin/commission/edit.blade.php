            ...
            <form method="post" action="{{ route('commission.update',$commission->id) }}" enctype="multipart/form-data">
                ...
                <div class="card">
                    <div class="card-header card_header">
                        <div class="row">
                            <div class="col-md-8">
                                <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> Update Commission</h4>
                            </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('category_id') ? ' has-error' : '' }}">
                            ...
                            <div class="col-sm-5">
                                <input name="category_id" value="{{ $commission->category_id }}" hidden>
                                <select disabled class="form-control select2" id="categoryId" name="category_id"  readonly="readonly" tabindex="-1" aria-disabled="true">
                                    <option value="">Select Parent Category</option>
                                    @foreach ($categories as $category)
                                        <option class="custom-style" value="{{ $category->id}}"
                                                style="font-weight: 900 !important; display: block !important; " {{ $category->id == $commission->category_id ? 'selected' : ''}} ><?= str_repeat("&nbsp; ", $category->level)?> {{  $category->category_name}}</option>
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-sm-5">
                                <select class="form-control vendors" multiple name="vendor_id[]">
                                    ...
                                    @foreach ($users as $key => $value )
                                        <option value="{{ $key }}"
                                                {{ $key == isset($categoryVendor[$count++])  ? 'selected' : ''}}
                            ...
                            @if($commission->is_all_vendor == 'true')
                                <div class="col-sm-3">
                                    <input id="allVendors" type="checkbox" style="margin-top:10px" name="is_all_vendor"
                                           value="true" {{ $commission->is_all_vendor == 'true' ? 'checked' : '' }} >
                                    <span style="font-size: 14px"> all vendors</span>
                                </div>
                            @endif
                        </div>
                        <div class="form-group row mb-3">
                            <label class="col-sm-4 col-form-label col_form_label text-left">Percentage <span
                                    class="req_star">*</span>:</label>
                            <div class="col-sm-5">
                                <input type="text"
                                       class="form-control form_control @error('percent') is-invalid @enderror"
                                       name="percent" value="{{old('percent',$commission->percent)}}">
                        ...
                        <div class="form-group row mb-3">
                            ...
                            <div class="col-sm-5">
                                <input type="date"
                                       class="form-control form_control @error('effectived_date') is-invalid @enderror"
                                       name="effectived_date" value="{{old('effectived_date',$commission->effectived_date)}}">
                                {{--@if ($errors->has('percent'))
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $errors->first('percent') }}</strong>
                                    </span>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#categoryId').on('change', function () {
                ...
                $.ajax({
                    method: "post",
                    url: "{{ url('commission-is-all-vendor') }}",
                    data: {
                        '_token': '{{ csrf_token() }}',

            ...
            <div class="card-header card_header">
                <div class="row">
                    ...
                    <div class="col-md-4 text-right">
                        <a href="{{ route('user.create') }}" class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i class="fas fa-plus-circle label-icon"></i>Add User</a>
                    </div>
                </div>
            </div>
            <div class="card-body">
...
{{--                            {{ dd($user->getRoleNames()) }}--}}
                            <td>@if(!empty($user->getRoleNames()))
                                    @foreach($user->getRoleNames() as $v)
                                        <label class="badge {{ $v == 'admin' ? 'badge-pink' : 'badge-success' }}">{{ $v }}</label>
                                    @endforeach
                                @endif</td>
{{--                            <td>--}}
{{--                              @if($data->photo!='')--}}
{{--                                <img class="img-thumbnail img40" src='{{("uploads/user/$data->image")}}'/>--}}
{{--                              @else--}}
{{--                                <img class="img-thumbnail img40" src="{{("uploads/user/$data->image")}}"/>--}}
{{--                              @endif--}}
{{--                            </td>--}}
                            <td><label class="btn {{ $user->status == 'APPROVED' ? 'text-success' : 'text-danger'}}">{{ $user->status }}</label></td>
                            <td>
                              <div class="btn-group">
                                ...
                                <div class="dropdown-menu">
                                    <a class="dropdown-item" href="{{ route("vendor.approve",$user->id) }}">View</a>
                                    <a class="dropdown-item" href='{{route("user.edit",$user->id)}}'>Edit</a>
                                    <a class="dropdown-item" href="#" id="softDelete" title="delete" data-toggle="modal" data-target="#softDelModal" data-id="{{$user->id}}">Delete</a>
                                </div>
                              </div>
                            </td>
                        </tr>

...
@section('content')
    <div class="row">
        <div class="col-12">
            <form method="post" action="{{ route('request-campaign-offer.accept',[$vendorDiscountCampaign->campaign_id,$vendorDiscountCampaign->id])}}" enctype="multipart/form-data">
                @csrf
                <div class="card">
                    <div class="card-header card_header">
                        <div class="row">
                            ...
                            <div class="col-md-4 text-right">
                                <a href="{{route('all-vendor-campaign.all')}}"
                                   class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i
                                        class="fas fa-th label-icon"></i>All discount campaign offer</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="form-group row mb-3 ">
                            ...
                            <div class="col-sm-6">
                                <input disabled type="text" autocomplete="off" class="form-control form_control"
                                       value="{{old('vendor_id',$vendor->ven->name)}}">
                            </div>
                        </div>
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('category_id') ? ' has-error' : '' }}">
                            <label class="col-sm-3 col-form-label col_form_label text-left"> Category name<span
                                    class="req_star">*</span>:</label>
                            <div class="col-sm-6">
                                <select disabled class="form-control select2 category" id="categoryId" name="" multiple="multiple">
                                    ...
                                    @foreach ($vendor->categories as $key => $category)
                                        <option class="custom-style" {{ (collect(old('category_id'))->contains($key)) ? 'selected':'' }}  value="{{ $key }}"
                                                @foreach(json_decode($vendorDiscountCampaign->category_id) as $val)
                                                {{ $key == $val ? 'selected' : '' }}
                                                @endforeach
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('product_id') ? ' has-error' : '' }}">
                            <label class="col-sm-3 col-form-label col_form_label text-left">Product Name<span
                                    class="req_star">*</span>:</label>
                            <div class="col-sm-6">
                                <select disabled class="form-control select2 product" id="productId" name="" multiple>
                                    <option value="">Select Products</option>
                                    <option value="all_product">All Product</option>
                                    @foreach ($vendor->products as $key => $product)
                                        <option class="custom-style"  value="{{ $key }}"
                                                @foreach(json_decode($vendorDiscountCampaign->product_id) as $val)
                                                {{ $key == $val ? 'selected' : '' }}
                                                @endforeach
                                                style="font-weight: 900 !important; display: block !important;"> {{  $product }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row mb-3 {{ $errors->has('percentage') ? ' has-error' : '' }}">
                            ...
                            <div class="col-3">
                                <select disabled class ="form-control " id ="discount_type" >
                    ...
                    <div class="card-footer card_footer text-center">
                        <button type="submit" class="btn btn-md btn-success font-weight-bold">Accepted Discount campaign</button>
                    </div>
                </div>
            </form>
        </div>

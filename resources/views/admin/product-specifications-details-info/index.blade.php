        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header card_header">
                    <div class="row">
                        <div class="col-md-7">
                            <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Product Specification Details Info</h4>
                        </div>
                        <div class="col-md-5 text-right">
                            <a href='{{ route("product-specification-info.create") }}'
                               class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i
                                    class="fas fa-plus-circle label-icon"></i>Add product specification details Info</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <table id="alltableinfo"
                        ...
                        <tr>
                            ...
                            <th>Specification Info Name</th>
                            <th>Status</th>
                            <th>Manage</th>
                        </tr>
                        </thead>
                        <tbody>
                        @foreach($productSpecificationDetailInfo as $data)
                            <tr>
                                <td>{{ isset($data->category) ? $data->category->category_name : '' }}</td>
                                <td>{{  $data->specification_detail_name ?: '' }}</td>
                                <td>
                                    @isset($data->name)
                                        @foreach(json_decode($data->name)  as $key => $val )
                                            <li>{{ $val }}</li>
                                        @endforeach
                                    @endisset
                                </td>
                                <td>
                                    <label class="badge {{ $data->status == 'active' ? 'badge-success' : 'badge-danger' }}">{{ $data->status  }}</label>
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <a class="dropdown-item"
                                               href="{{ route('product-specification-info.edit',$data->id) }}">Edit</a>

        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header card_header">
                    <div class="row">
                        <div class="col-md-8">
                            <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Feature Name</h4>
                        </div>
                        <div class="col-md-4 text-right">
                            <a href='{{ route("feature-name.create") }}'
                               class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i
                                    class="fas fa-plus-circle label-icon"></i>Add Feature Name</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <table id="alltableinfo"
                        ...
                        <tbody>
                        @foreach( $featureNames as $data)
                            <tr>
                                <td>{{ $data->name ?: '' }}</td>
                                <td>@if($data->display_products == 7)
                                        <label class="text-success"> {{ 'Feature Products' }}</label>
                                    @elseif($data->display_products == 5)
                                        <label class="text-info">{{ 'Slider Products' }}</label>
                                    @elseif($data->display_products == 3)
                                        <label class="text-pink">{{ "Selected Products" }}</label>
                                    @elseif($data->display_products == 2)
                                        <label style="color: #c08009">{{ "Special Products" }}</label>
                                    @else
                                        {{ '--' }}
                                    @endif
                                </td>
                                <td>{{ $data->display_products ?: '' }}</td>

                                <td>@if($data->display_type == null)
                                        {{ ' -- ' }}
                                    @else
                                        {{ $data->display_type== 0 ? 'left' : 'right' }}
                                    @endif
                                </td>

                                <td>{{ $data->display_serial ?: '' }}</td>
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <a class="dropdown-item" href="{{ route('feature-name.edit',$data->id) }}">Edit</a>

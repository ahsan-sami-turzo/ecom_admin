        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header card_header">
                    <div class="row">
                        <div class="col-md-8">
                            <h4 class="card-title card_title"><i class="fab fa-gg-circle"></i> All Purchase return</h4>
                        </div>
                        <div class="col-md-4 text-right">
                            <a href='{{ route("purchase-returns.create") }}'
                               class="btn btn-dark btn-md waves-effect btn-label waves-light card_btn"><i
                                    class="fas fa-plus-circle label-icon"></i>Add Purchase return</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    ...
                    <table id="alltableinfo"
                        ...
                        @foreach($purchaseReturns as $data)
                            <tr>
                                <td>{{ $data->purchaseReturnBillNo ?? '' }}</td>
                                <td>{{ date('d-m-Y',strtotime($data->purchaseReturnDate)) ?? '' }}</td>
                                <td>{{ $data->totalQuantity ?: '' }}</td>
                                <td>{{ $data->totalAmount ?: '' }}</td>
                                <td>
                                    <label class="p-2 badge {{ $data->status == 1 ? 'badge-success' : 'badge-danger' }}">{{ $data->status == 1 ? 'Active' : 'Inactive' }}</label>
                                </td>
                                <td>
                                    <div class="btn-group">
                                        <button class="btn btn-dark btn-sm" type="button">Manage</button>
                                        <button type="button" class="btn btn-sm btn-secondary dropdown-toggle dropdown-toggle-split waves-effect btn-label waves-light card_btn"
